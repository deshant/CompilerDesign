...
bool match(entry tk)
{
	if(lookahead.token == 0)
		lookahead = analyze();
	return (lookahead.token == tk.token);
}
// update lookahead
void advance()
{
...
entry analyze()
{
	...
	switch(statement[statement_index]){
		...
		case ')':
			...
			symbol.lexeme[1] = '\0';
			symbol.token = OPERATOR;
			return symbol;
		default:
			break;
	}
	if(is_operator(statement[statement_index])){
		symbol.lexeme[0] = statement[statement_index++];
		symbol.lexeme[1] = '\0';
		symbol.token = OPERATOR;
		return symbol;
	...
	// found number
	else if(isdigit(statement[statement_index])){
		int j = 0;
		do{
	...
	// found identifier
		...
		if(t == -1){
			insert(symbol.lexeme,IDENTIFIER);
		}
